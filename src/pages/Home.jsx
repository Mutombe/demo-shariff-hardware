    ...
    <section ref={containerRef} className="relative h-screen min-h-[700px] overflow-hidden bg-slate-950">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <AnimatePresence mode="sync">
          <motion.img key={currentSlide} src={heroImages[currentSlide]} alt="Construction materials"
            ...
            transition={{ duration: 2 }} loading="eager" />
        </AnimatePresence>
        <div className="absolute inset-0 bg-slate-950/80 z-[1]" />
      </motion.div>
      <NoiseTexture opacity={0.04} />
      <motion.div className="relative z-20 flex flex-col items-center justify-center text-center h-full max-w-5xl mx-auto px-5 sm:px-8 lg:px-12 pt-36" style={{ opacity }}>
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', delay: 0.3 }}
          className="w-14 h-14 bg-blue-600 flex items-center justify-center mb-8 shadow-xl">
          <Buildings size={24} weight="fill" className="text-white" />
        </motion.div>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
          className="text-blue-400/70 text-xs font-semibold uppercase tracking-[0.3em] mb-6" style={{ fontFamily: 'var(--font-sans)' }}>
          {hero.badge}
        </motion.p>
        <div className="overflow-hidden">
          <motion.h1 initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ duration: 1, delay: 0.6, ease: [0.16, 1, 0.3, 1] }}
            className="font-heading text-white leading-[0.92] tracking-tight" style={{ fontSize: 'clamp(2.2rem, 7vw, 4.5rem)' }}>
            BUILD IT <span className="text-blue-500 italic font-bold">RIGHT</span><br />THE FIRST TIME
          </motion.h1>
        </div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.1 }} className="flex items-center justify-center gap-6 mt-8">
          {[{ val: business.projectsCompleted, lbl: 'Customers' }, { val: business.yearsExperience, lbl: 'Experience' }, { val: '5000+', lbl: 'Products' }].map((s, i) => (
            <div key={i} className="text-center">
              <div className="text-blue-500 font-heading text-xl sm:text-2xl italic">{s.val}</div>
              <div className="text-white/25 text-[9px] uppercase tracking-wider mt-1" style={{ fontFamily: 'var(--font-sans)' }}>{s.lbl}</div>
            </div>
          ))}
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.4 }} className="flex flex-wrap justify-center gap-4 mt-10">
          <Link to="/contact" className="group inline-flex items-center gap-3 bg-blue-600 text-white px-8 py-4 text-sm uppercase tracking-[0.15em] font-semibold hover:bg-blue-700 transition-all" style={{ fontFamily: 'var(--font-sans)' }}>
            ...
            return (
              <motion.div key={service.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                ...
                <Link to={`/services#${services?.items?.[i]?.slug || ''}`}
                  className={`group relative block overflow-hidden ${i === 0 ? 'aspect-[2/1]' : 'aspect-[3/4]'}`}>
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
            ...
            <h2 className="font-heading text-white leading-[0.95] italic mb-8" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)' }}>
              The Shariff<br /><span className="text-blue-500">Story</span>
            </h2>
            {about.story.map((p, i) => (
        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={inView ? { opacity: 1, x: 0 } : {}}
            ...
            <h2 className="font-heading text-white leading-[0.95] italic mb-12" style={{ fontSize: 'clamp(2rem, 4.5vw, 3rem)' }}>
              Why Choose <span className="text-blue-500">Shariff</span>
            </h2>
...
function CTASection() {
  ...
  return (
    <section ref={ref} className="relative py-28 sm:py-36 lg:py-48 overflow-hidden">
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        <img src={siteData.hero.backgroundImages[0]?.url} alt="Shariff Hardware" className="w-full h-[130%] object-cover object-center" loading="lazy" />
